...
function normalizePhone($phone) {
    ...
    if (strpos($phone, '+880') === 0 && strlen($phone) === 14) {
        return $phone;
    }

    // Remove + if exists but not with 880
    $phone = ltrim($phone, '+');

    // If starts with 880
    if (strpos($phone, '880') === 0) {
        return '+880' . substr($phone, 3);
    }

    // If starts with 0 (most common case)
    if (strpos($phone, '0') === 0) {
        $phone = substr($phone, 1);
    }

    // Now we should have 10 or 11 digits
    ...
    // Fallback - take last 11 digits
    if (strlen($phone) > 11) {
        $last11 = substr($phone, -11);
        if (strpos($last11, '1') === 0) {
            return '+880' . $last11;
        }
    }
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'get_units') {
    ...
    if ($selected_unit > 0) {
        $sql = "SELECT id, unit_name, rent, advance FROM unit 
        ...
        if ($status === 'Booked') {
            $sql = "SELECT id, unit_name, rent, advance FROM unit 
                    WHERE building_name = $building_id ORDER BY unit_name ASC";
        } else {
            $sql = "SELECT id, unit_name, rent, advance FROM unit 
                    WHERE building_name = $building_id AND status = 'Available' 
                    ORDER BY unit_name ASC";
        }
    }
...
if (isset($_GET['edit_id'])) {
    $id = (int)$_GET['edit_id'];
    $q = mysqli_query($db, "SELECT * FROM tenants WHERE role = 'Tenant' AND id=$id");
...
if (isset($_POST['save_tenant'])) {
    ...
    $phone           = normalizePhone($phone_raw);           // ← Normalized
    ...
    $phone_check_sql = "SELECT id FROM tenants WHERE phone = '$phone' AND role = 'Tenant'";
    if ($id > 0) {
        $phone_check_sql .= " AND id != $id";
    }
    $phone_check = mysqli_query($db, $phone_check_sql);

    if (mysqli_num_rows($phone_check) > 0) {
        $message = "<div class='alert alert-danger'> ❌ This phone number is already registered.<br> <strong>Phone Number:</strong> $phone </div>";
    } else {
        // Image Upload
        $tenant_img = $_POST['old_tenant_image'] ?? '';
        if (!empty($_FILES['tenant_image']['name'])) {
            $tenant_img = time() . '_' . basename($_FILES['tenant_image']['name']);
        ...
        if (!empty($_FILES['nid_image']['name'])) {
            $nid_img = time() . '_' . basename($_FILES['nid_image']['name']);
            move_uploaded_file($_FILES['nid_image']['tmp_name'], "public/uploads/nid/" . $nid_img);
        }

        if ($id) {
            // UPDATE
            if ($unit != $old_unit_id && $unit > 0) {
            ...
            mysqli_query($db, "
                INSERT INTO tenants 
                (name, phone, email, status, role, booking_month, permanent_address, family_member, 
                 tenant_image, nid_image, building_id, unit_id, start_tanent, nid_no)
                VALUES 
                ('$name','$phone','$email', '$status', 'Tenant', '$booking_month', '$address',
                 '$family','$tenant_img','$nid_img','$building','$unit','$start_tanent','$nid_no')
            ");
...
<div class="container mb-3 px-4">
    ...
    <form method="POST" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="month" name="booking_month" class="form-control">
        </div>
        <?php else: ?>
...
<script>
    ...
    $(document).ready(function() {
        loadUnits(<?= (int)$editData['building_id'] ?>, <?= (int)$editData['unit_id'] ?>);
    });
    <?php endif; ?>
</script>