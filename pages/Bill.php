        ...
        <div class="mobile-card-view">
            ...
            <div class="mobile-unit-card position-relative">
                ...
                <div class="status-badge-mobile">
                    <span class="badge bg-<?= ($status=='Paid')?'success':(($status=='Unpaid')?'danger':'warning') ?>-subtle text-<?= ($status=='Paid')?'success':(($status=='Unpaid')?'danger':'warning') ?> border border-<?= ($status=='Paid')?'success':(($status=='Unpaid')?'danger':'warning') ?>">
                        <?= $status ?>
                    </span>
                </div>
                ...
                <div class="row g-2 text-center mb-3 border-top pt-2">
                    <div class="col-4 border-end">
                        ...
                        <div class="fw-bold text-primary">৳<?= number_format($total_amount_db, 0) ?></div>
                    </div>
                    <div class="col-4 border-end">
                        <div class="text-muted small">Paid</div>
                        <div class="fw-bold text-success">৳<?= number_format($paid_amount_db, 0) ?></div>
                    </div>
                    <div class="col-4">
                        <div class="text-muted small">Due</div>
                        <div class="fw-bold text-danger">৳<?= number_format($due_amount_db, 0) ?></div>
                    </div>
                </div>

                <div class="d-flex gap-2">
                    <a href="admin.php?page=editbill&unit_id=<?= $unit_id ?>" class="btn btn-outline-info btn-sm flex-grow-1 py-2 rounded-pill">
                        <i class="bi bi-eye me-1"></i> Details
                    </a>
                    <a href="admin.php?page=bill&unit_id=<?= $unit_id ?>&id=<?= $building_name ?>" onclick="sendWhatsApp()" class="btn btn-success btn-sm flex-grow-1 py-2 rounded-pill">
                        <i class="bi bi-whatsapp me-1"></i> Send Bill
                    </a>
                </div>
            </div>
            <?php } ?>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <a href="admin.php?page=view_tenant&id=<?= $tent_id ?>" class="text-secondary small"><?= $name ?></a>
                                    </div>
                                </div>
                            </td>
                            <td class="small">
                                <?php
                                    ...
                                    if(mysqli_num_rows($pay_info) > 0){
                                        ...
                                        echo "<span class='text-primary'>Total: ৳".number_format($p['total_amount'],0)."</span><br>";
                                        echo "<span class='text-success'>Paid: ৳".number_format($p['paid_amount'],0)."</span><br>";
                                        echo "<span class='text-danger fw-bold'>Due: ৳".number_format($p['total_amount']-$p['paid_amount'],0)."</span>";
                                        $status = $p['status'];
                                    } else {
                            ...
                            <td>
                                <span class="badge bg-<?= ($status=='Paid')?'success':(($status=='Unpaid')?'danger':'warning') ?>"><?= $status ?></span>
                            </td>
                            <td>
                                <!-- Payment method and manager logic (keeping your logic) -->
                                <?php
                                    ...
                                    if(mysqli_num_rows($history_sql) > 0) {
                                        echo "<small class='text-muted'>Multiple Txn Found</small>";
                                    } else {
                            ...
                            <td class="text-end pe-3">
                                <div class="btn-group">
                                    <a href="admin.php?page=editbill&unit_id=<?= $unit_id ?>" class="btn btn-sm btn-info text-white">Details</a>
                                    <a href="#" onclick="sendWhatsApp()" class="btn btn-sm btn-success"><i class="bi bi-whatsapp"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php } ?>
