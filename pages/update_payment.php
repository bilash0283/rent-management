...
$old_history_paid_amount = (float) $history['paid_amount']; // এই নির্দিষ্ট পেমেন্ট ট্রানজেকশনে আগে কত ছিল
...
if (isset($_POST['update_payment'])) {
    ...
    // মেইন ইনভয়েসের নতুন সম্ভাব্য টোটাল জমা: (বর্তমান টোটাল - এই এন্ট্রির আগের ভ্যালু) + নতুন ইনপুট ভ্যালু
    $adjusted_invoice_paid = ($invoice_paid_total - $old_history_paid_amount) + $new_paid_amount;

    // --- নতুন কন্ডিশন: টোটাল বিলের চেয়ে বেশি পেমেন্ট কি না ---
    if ($adjusted_invoice_paid > $invoice_total_bill) {
        $max_allowed = $invoice_total_bill - ($invoice_paid_total - $old_history_paid_amount);
        echo "<script>
            alert('Error: Total paid amount cannot exceed the Total Bill ($invoice_total_bill)! You can pay maximum $max_allowed for this transaction.');
            window.history.back();
        </script>";
        exit();
    }
...
<div class="nxl-content mx-3">
    <div class="card mt-4 shadow-sm">
        <div class="card-header bg-light text-white d-flex justify-content-between align-items-center">
            <h6 class="mb-0 text-black">Edit Payment Transaction (Total Bill: <?= $invoice_data['total_amount'] ?>)</h6>
            ...
            <form method="POST">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" step="any" name="paid_amount" class="form-control border-primary" value="<?= $old_history_paid_amount ?>" required>
                    </div>
                    ...
                    <div id="manager_section" style="display: none;" class="p-3 border rounded bg-light mb-3 shadow-sm">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                ?>
                                <input type="number" step="any" name="manager_payment" id="manager_payment" class="form-control" value="<?= $current_manager_payment ?>">
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row g-3 mt-1" id="expense_row">
                            <div class="col-md-6">
                                ...
                                <input type="number" step="any" name="expense" class="form-control" value="<?= (float)$history['expense'] ?>">
                            </div>
                            <div class="col-md-6">
...
<script>
    ...
    function toggleManagerTransaction() {
        ...
        if (managerMethod === "Cash" || managerMethod === "" || !managerTransactionDiv) {
            if(managerTransactionDiv) managerTransactionDiv.style.display = 'none';
        } else {
